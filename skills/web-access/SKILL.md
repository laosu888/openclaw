...
```bash
bash /root/.openclaw/workspace/skills/web-access/scripts/check-deps.sh
```
...
已有经验的站点：!`ls /root/.openclaw/workspace/skills/web-access/references/site-patterns/ 2>/dev/null | sed 's/\.md$//' || echo "暂无"`
